...
def get_motion_module(
    ...
    if motion_module_type == "Vanilla":
        return VanillaTemporalModule(in_channels=in_channels, level=level, **motion_module_kwargs,)    
    else:
        raise ValueError
...
class TemporalTransformer3DModel(nn.Module):
    def __init__(
        ...
        super().__init__()
        inner_dim = num_attention_heads * attention_head_dim # 8 * 
        # that's hack in order to use the pretrained weights
        
        self.norm = torch.nn.GroupNorm(num_groups=norm_num_groups, num_channels=in_channels, eps=1e-6, affine=True)
        self.proj_in = nn.Linear(in_channels, inner_dim)
        # that's hack in order to use the pretrained weights
        #if level == 4 or 2 or 5:
        ...
        if self.video_length is None:
            raise ValueError("video_length is not provided")
