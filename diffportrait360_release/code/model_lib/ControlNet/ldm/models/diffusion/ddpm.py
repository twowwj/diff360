...
class LatentDiffusionReferenceOnly(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if 'more_image_control' in cond and cond['more_image_control'] is not None:
            num_additional_ref_imgs = len(cond['more_image_control'])
            for i in range(num_additional_ref_imgs):
                m_ref_img_noisy = cond['more_image_control'][i]
                if not cond['wonoise']:
        ...
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        #import pdb;pdb.set_trace()
        model_output = self.apply_model(x_noisy, t, cond, reference_image_noisy, more_reference_image_noisy = more_reference_image_noisy)
