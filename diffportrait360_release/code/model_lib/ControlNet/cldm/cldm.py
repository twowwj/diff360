...
from model_lib.ControlNet.ldm.modules.diffusionmodules.openaimodel import UNetModel, TimestepEmbedSequential, ResBlock, Downsample, AttentionBlock,Upsample, UNetModel_Temporal
        ...
        if uc:
            ...
            for i, module in enumerate(self.output_blocks):
                ...
                if only_mid_control or (bank_attn is None):
                    #print("bank_attn", bank_attn)
                    h = torch.cat([h, hs.pop()], dim=1)
                    h = module(h, emb, context) 
                else:
                    if  pose_control is not None:
                        
                        #print(h.shape, hs[0].shape, pose_control[0].shape)
                        #pdb.set_trace()
...
class ControlLDMReferenceOnlyPose(LatentDiffusionReferenceOnly):
    ...
    def apply_model(self, x_noisy, t, cond, reference_image_noisy, more_reference_image_noisy = None, uc=False, *args, **kwargs):
        ...
        # pdb.set_trace()
        attention_bank = []
        #ab = self.decode_first_stage(reference_image_noisy)
        #import pdb;pdb.set_trace()
        if reference_image_noisy is not None:
            empty_outs = self.control_model(x=reference_image_noisy, hint=None, timesteps=t, context=cond_txt, attention_bank=attention_bank, attention_mode='write', uc=uc)
        #for i in range(len(attention_bank)):
        #    attention_bank[i] = torch.cat([attention_bank[i], attention_bank[i]], dim=1)
        #import pdb;pdb.set_trace()  
        if  more_reference_image_noisy is not None: #is not None:
            for m_reference_image_noisy in more_reference_image_noisy:
                l_attention_bank = []
                #import pdb;pdb.set_trace()
                empty_outs = self.control_model(x=m_reference_image_noisy, hint=None, timesteps=t, context=cond_txt_void, attention_bank=l_attention_bank, attention_mode='write',uc=uc)
                
                for j in range(len(attention_bank)):
                    attention_bank[j] = torch.concat([attention_bank[j], l_attention_bank[j]], dim=1)
                    #print("using more reference, trying to see if they have gradients")
                        
            #import pdb;pdb.set_trace()
                        
            #             #attention_bank[j] = torch.concat(tmp_bank)
        #import pdb;pdb.set_trace()
        
        #if self.control_enabled and 'c_concat' in cond and cond['c_concat'] is not None:
            #import pdb;pdb.set_trace()
        cond_hint = torch.cat(cond['c_concat'], 1)
        #print("using fuckign control net hi!!!!!!!")
        pose_control = self.pose_control_model(x=x_noisy, hint=cond_hint, timesteps=t, context=cond_txt)
        
        #pose_control = None
        #print("attention!!1  Pose control is None")
